...
static int isGotRtcpSr = 0;  // 20121020 Mingta
...
static int rtcp_parse_packet(RTPDemuxContext *s, const unsigned char *buf, int len)
{
    ...
    while (len >= 4) {
        ...
        case RTCP_SR:
            ...
            //printf("s->last_rtcp_ntp_time=%d\n", s->last_rtcp_ntp_time);
            //printf("s->last_rtcp_timestamp=%d\n", s->last_rtcp_timestamp);
            ...
            //printf("s->first_rtcp_ntp_time=%d\n", s->first_rtcp_ntp_time);
            //printf("s->base_timestamp=%d\n", s->base_timestamp);
						av_log(NULL, AV_LOG_DEBUG, "isGotRtcpSr\n");
						isGotRtcpSr = 1;
...
int ff_rtp_check_and_send_back_rr(RTPDemuxContext *s, int count)
{
    ...
    if(s->octet_count < s->last_octet_count) {
    	s->octet_count = 0;
    	s->last_octet_count = 0;
    }
    ...
    // 20121020 Mingta: workaround for Uniform IP CAM. Force reply for RTCP_SR.
    if(isGotRtcpSr == 0) {
    	if (rtcp_bytes < 28)
        return -1;
    ...
    //if(s->octet_count > 100000000) {
    //		s->octet_count = 0;
    //}
    
    av_log(NULL, AV_LOG_DEBUG, "last_octet_count=%d, octet_count=%d, rtcp_bytes=%d\n", s->last_octet_count, s->octet_count, rtcp_bytes);
    ...
    if ((len > 0) && buf) {
        ...
        av_log(NULL, AV_LOG_DEBUG, "sending %d bytes of RR\n", len);
...
static int ffmpegPacketLostCount = 0;
int Ffmpeg_GetPacketLostCount(void);
int Ffmpeg_GetPacketLostCount(void) {
	return ffmpegPacketLostCount;
}
...
static int rtp_parse_queued_packet(RTPDemuxContext *s, AVPacket *pkt)
{
    ...
    if (!has_next_packet(s)) {
        av_log(s->st ? s->st->codec : NULL, AV_LOG_WARNING,
               "RTP(Mingta): missed %d packets\n", s->queue->seq - s->seq - 1);
        ffmpegPacketLostCount += s->queue->seq - s->seq - 1;
    }
